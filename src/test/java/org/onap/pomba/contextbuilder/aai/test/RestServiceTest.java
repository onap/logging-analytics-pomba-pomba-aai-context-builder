...
public class RestServiceTest {
    ...
    private static MultivaluedMap<String, String> buildHeaders(
            String partnerName, String requestId, String authorization) {

        MultivaluedMap<String, String> headers = new MultivaluedHashMap<>();
        headers.put(RestUtil.FROM_APP_ID, Collections.singletonList(partnerName));
        headers.put(RestUtil.TRANSACTION_ID, Collections.singletonList(requestId));
    ...
    @Test
    public void testGetContextVserverPserver() throws Exception {

        String requestId = UUID.randomUUID().toString();
        String serviceInstanceId = "adc3cc2a-c73e-414f-8ddb-367de81300cb";
        ...
        addResponse("/aai/v13/business/customers/customer/DemoCust_651800ed-2a3c-45f5-b920-85c1ed155fc2"
                  + "/service-subscriptions/service-subscription/vFW/service-instances/service-instance/"
                  + "adc3cc2a-c73e-414f-8ddb-367de81300cb",
                    "junit/aai-service-instance_set2.json",
                    aaiEnricherRule);

        // 3. simulate the rsp of VNF (with 1 vserver)
        // note: match vnf_id in (2)
        addResponse("/aai/v13/network/generic-vnfs/generic-vnf/8a9ddb25-2e79-449c-a40d-5011bac0da39"
                  + DEPTH,
                    "junit/genericVnfInput_set2.json",
                    aaiEnricherRule);

        // 4. simulate the rsp of vserer
        // note: match to vserver-id to the path of "vserver" in (3)
        addResponse("/aai/v13/cloud-infrastructure/cloud-regions/cloud-region/CloudOwner/RegionOne/tenants/tenant"
        ...
        addResponse("/aai/v13/cloud-infrastructure/pservers/pserver/mtn96compute.cci.att.com"
                  + DEPTH,
                    "junit/pserverInput_set2.json",
                    aaiEnricherRule);

        when(mockHttpHeaders.getRequestHeaders()).thenReturn(multivaluedMapImpl);

        when(mockHttpServletRequest.getRemoteAddr()).thenReturn("testGetContextVserverPserver");

        Response response = this.dummyRestSvc.getContext(mockHttpHeaders,
    ...
    // serviceInstanceId -> vnf (containing L-Interface) -> vserver (containing - LInterfaceList)

    @Test
    public void testGetContextVserverLinterfaceAndLogicalLink() throws Exception {
        ...
        addResponse("/aai/v13/network/generic-vnfs/generic-vnf/8a9ddb25-2e79-449c-a40d-5011bac0da39"
                  + DEPTH,
                    "junit/genericVnfInput_set6.json",
                    aaiEnricherRule);
        ...
        addResponse("/aai/v13/network/logical-links/logical-link/01e8d84a-logical-link-1",
                    "junit/logical-link-input1.json",
                    aaiEnricherRule);
        addResponse("/aai/v13/network/logical-links/logical-link/01e8d84a-logical-link-2",
                    "junit/logical-link-input2.json",
                    aaiEnricherRule);
        addResponse("/aai/v13/network/logical-links/logical-link/01e8d84a-logical-link-3",
                    "junit/logical-link-input3.json",
                    aaiEnricherRule);
        addResponse("/aai/v13/network/logical-links/logical-link/01e8d84a-logical-link-4",
                    "junit/logical-link-input4.json",
                    aaiEnricherRule);


        when(mockHttpHeaders.getRequestHeaders()).thenReturn(multivaluedMapImpl);

        when(mockHttpServletRequest.getRemoteAddr()).thenReturn("testGetContextVserverLinterfaceAndLogicalLink");
        ...
        assertEquals(vnfList.get(0).getLInterfaceList().get(0).getName(),
                     "junit-l-interface-name3"); //l-interface-name
        assertEquals(vnfList.get(0).getLInterfaceList().get(1).getName(),
                     "junit-l-interface-name4"); //l-interface-name
        assertEquals(vnfList.get(0).getLInterfaceList().get(0).getLogicalLinkList().size(), 1);
        assertEquals(vnfList.get(0).getLInterfaceList().get(0).getLogicalLinkList().get(0).getName(),
                     "01e8d84a-logical-link-3"); //Logical-link at vnf->l-interface -> Logical-link
        assertEquals(vnfList.get(0).getLInterfaceList().get(1).getLogicalLinkList().size(), 1);
        assertEquals(vnfList.get(0).getLInterfaceList().get(1).getLogicalLinkList().get(0).getName(),
                     "01e8d84a-logical-link-4");
        List<VFModule>  vfModuleList = vnfList.get(0).getVfModules();
        assertEquals(vfModuleList.size(), 1);
        List<VM> vmList = vfModuleList.get(0).getVms();
        assertEquals(vmList.size(), 1);
        ...
        assertEquals(vmList.get(0).getLInterfaceList().get(0).getName(),
                     "junit-l-interface-name1"); //l-interface-name
        assertEquals(vmList.get(0).getLInterfaceList().get(1).getName(),
                     "junit-l-interface-name2"); //l-interface-name
        assertEquals(vmList.get(0).getLInterfaceList().get(0).getLogicalLinkList().size(), 1);
        assertEquals(vmList.get(0).getLInterfaceList().get(0).getLogicalLinkList().get(0).getName(),
                     "01e8d84a-logical-link-1"); //Logical-link at vserver->l-interface -> Logical-link
        assertEquals(vmList.get(0).getLInterfaceList().get(1).getLogicalLinkList().size(), 1);
        assertEquals(vmList.get(0).getLInterfaceList().get(1).getLogicalLinkList().get(0).getName(),
                     "01e8d84a-logical-link-2");

    }

    ///Verify the relationship serviceInstanceId -> vnf + vnfc
    @Test
    public void testGetContextVnfc() throws Exception {
        ...
        addResponse("/aai/v13/network/generic-vnfs/generic-vnf/8a9ddb25-2e79-449c-a40d-5011bac0da39"
                  + DEPTH,
                    "junit/genericVnfInput_set5.json",
                    aaiEnricherRule);
        ...
        addResponse("/aai/v13/cloud-infrastructure/cloud-regions/cloud-region/CloudOwner/RegionOne/tenants/tenant"
                  + "/b49b830686654191bb1e952a74b014ad/vservers/vserver/b494cd6e-b9f3-45e0-afe7-e1d1a5f5d74a"
                  + DEPTH,
                    "junit/aai-vserver-set2.json",
                    aaiEnricherRule);

        // 5. simulate the rsp of vnfc
        // note: match to vnfc-name to the path of "vnfc" in (3)
        addResponse("/aai/v13/network/vnfcs/vnfc/junit-vnfc-name1212",
                    "junit/vnfc-input1.json",
                    aaiEnricherRule);

        when(mockHttpHeaders.getRequestHeaders()).thenReturn(multivaluedMapImpl);

        when(mockHttpServletRequest.getRemoteAddr()).thenReturn("testGetContextVnfc");
    ...
    ///Verify the relationship serviceInstanceId -> vnf + vnfc
    @Test
    public void testGetContextVnfc2() throws Exception {
        ...
        when(mockHttpServletRequest.getRemoteAddr()).thenReturn("testGetContextVnfc2");
    ...
    ///Verify the relationship serviceInstanceId -> vnf + vnfc
    @Test
    public void testGetContextLogicalLinkInServiceLevel() throws Exception {
        String requestId = UUID.randomUUID().toString();
        String serviceInstanceId = "adc3cc2a-c73e-414f-8ddb-367de81300cb";
        String queryNodeUrl = aaiPathToSearchNodeQuery + serviceInstanceId;
        ...
        addResponse("/aai/v13/business/customers/customer/DemoCust_651800ed-2a3c-45f5-b920-85c1ed155fc2"
                  + "/service-subscriptions/service-subscription/vFW/service-instances/service-instance/"
                  + "adc3cc2a-c73e-414f-8ddb-367de81300cb",
                    "junit/aai-service-instance_set4.json",
                    aaiEnricherRule);
        ...
        when(mockHttpServletRequest.getRemoteAddr()).thenReturn("testGetContextLogicalLinkInServiceLevel");
    ...
    @Test
    public void testGetContextLogicalLinkInPInterfaceLevelWithPnf() throws Exception {
        String requestId = UUID.randomUUID().toString();
        String serviceInstanceId = "adc3cc2a-c73e-414f-8ddb-367de81300cb";
        String queryNodeUrl = aaiPathToSearchNodeQuery + serviceInstanceId;
        ...
        addResponse("/aai/v13/network/pnfs/pnf/amdocsPnfName"
                  + DEPTH,
                   "junit/pnfInput_w_pInterface_LInterface_set2.json",
                   aaiEnricherRule);
        ...
        when(mockHttpServletRequest.getRemoteAddr()).thenReturn("testGetContextLogicalLinkInPInterfaceLevelWithPnf");
        ...
        assertEquals(modelCtx.getPnfs().get(0).getPInterfaceList().get(0).getLInterfaceList().get(0).getName(),
                     "junit-l-interface-name5"); //l-interface-name
        assertEquals(modelCtx.getPnfs().get(0).getPInterfaceList().get(0).getLInterfaceList().get(1).getName(),
                     "junit-l-interface-name6"); //l-interface-name
        assertEquals(modelCtx.getPnfs().get(0).getPInterfaceList().get(0).getLogicalLinkList().size(), 2);
        assertEquals(modelCtx.getPnfs().get(0).getPInterfaceList().get(0).getLogicalLinkList().get(0).getName(),
                     "01e8d84a-logical-link-1");
        assertEquals(modelCtx.getPnfs().get(0).getPInterfaceList().get(0).getLogicalLinkList().get(1).getName(),
                     "01e8d84a-logical-link-2");
    }

    ///Verify the relationship P-interface, L-interaface, Logical-link
    @Test
    public void testGetContextLogicalLinkInPInterfaceLevelWithPserver() throws Exception {
        String requestId = UUID.randomUUID().toString();
        String serviceInstanceId = "adc3cc2a-c73e-414f-8ddb-367de81300cb";
        String queryNodeUrl = aaiPathToSearchNodeQuery + serviceInstanceId;
        ...
        addResponse("/aai/v13/cloud-infrastructure/pservers/pserver/mtn96compute.cci.att.com"
                  + DEPTH,
                   "junit/pserverInput_with_pInterface_LInterface_set2.json",
                   aaiEnricherRule);

        // 6. simulate the rsp of logical-link
        // note: match to link-name to the path of "relationship-list" in
        // (5:  pserverInput_with_pInterface_LInterface_set2)
        addResponse("/aai/v13/network/logical-links/logical-link/01e8d84a-logical-link-1",
                    "junit/logical-link-input1.json",
                    aaiEnricherRule);
        addResponse("/aai/v13/network/logical-links/logical-link/01e8d84a-logical-link-2",
        ...
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getName(),
                     "bdc3cc2a-c73e-414f-7ddb-367de92801cb"); //interface-name
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLInterfaceList().size(), 2);
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLInterfaceList().get(0).getName(),
                     "junit-l-interface-name7"); //Vserver-> Pserver-> P-interface -> l-interface
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLInterfaceList().get(1).getName(),
                     "junit-l-interface-name8");
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLInterfaceList().get(0)
                     .getLogicalLinkList().size(), 1);
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLInterfaceList().get(0)
                     .getLogicalLinkList().get(0).getName(),
                     "01e8d84a-logical-link-3");  //Vserver-> Pserver-> P-interface -> l-interface -> logical-link
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLInterfaceList().get(1)
                     .getLogicalLinkList().size(), 1);
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLInterfaceList().get(1)
                     .getLogicalLinkList().get(0).getName(),
                     "01e8d84a-logical-link-4");
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLogicalLinkList().size(), 2);
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLogicalLinkList().get(0).getName(),
                     "01e8d84a-logical-link-1"); //Vserver-> Pserver-> P-interface -> Logical-link
        assertEquals(vmList.get(0).getPServer().getPInterfaceList().get(0).getLogicalLinkList().get(1).getName(),
                     "01e8d84a-logical-link-2");
    }
}
